            ...
            <div className="lms-page-container">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '30px' }}>
                    {courses.length === 0 ? (
                        ...
                        courses.map(c => (
                            <div key={c.id} className="dashboard-card hover-card" style={{ cursor: 'pointer' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                                    <div style={{ background: '#eff6ff', color: '#2196F3', padding: '12px', borderRadius: '16px' }}>
                                        <span className="material-icons-round" style={{ fontSize: '32px' }}>school</span>
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ fontSize: '0.8rem', fontWeight: 800, color: '#94a3b8' }}>{c.code}</div>
                                        <div style={{ fontSize: '0.7rem', color: '#2196F3', fontWeight: 700, textTransform: 'uppercase', marginTop: '4px' }}>{c.semester}</div>
                                    </div>
                                </div>
                                <h3 style={{ fontSize: '1.25rem', fontWeight: 900, color: '#1e293b', marginBottom: '10px' }}>{c.name}</h3>
                                <p style={{ fontSize: '0.9rem', color: '#64748b', lineHeight: '1.6', height: '4.8em', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
